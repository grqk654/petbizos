    ...
    <div>
      ...
      <div style={{ background: C.bgWarm, borderBottom: `1px solid ${C.amberBorder}`, padding: '72px 24px 64px' }}>
        <div style={{ maxWidth: 740, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 20, flexWrap: 'wrap' }}>
            {['Free for pet owners','77 Brooklyn providers','Real Google data'].map(t => (
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '52px 24px 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 52 }}>
          {[
            { cat:'Trainers',    icon:'🎓', count:31, color:C.amberLight, border:C.amberBorder },
            { cat:'Daycare',     icon:'☀', count:28, color:C.blueLight,  border:C.blueBorder },
            { cat:'Dog Walkers', icon:'🦮', count:13, color:C.greenLight, border:'#C0DD97' },
            { cat:'Groomers',    icon:'✂', count:3,  color:C.amberLight, border:C.amberBorder },
            { cat:'Boarding',    icon:'🏠', count:2,  color:C.blueLight,  border:C.blueBorder },
            { cat:'Vets',        icon:'🏥', count:0,  color:C.greenLight, border:'#C0DD97' },
            ...
            <button key={cat} onClick={() => setPage('directory')}
              ...
              <div style={{ fontFamily: F.mono, fontSize: 12, color: C.textMuted }}>{count > 0 ? `${count} in Brooklyn` : 'Coming soon'}</div>
            </button>
          ))}
        </div>
...
const DirectoryPage = ({ setPage }) => {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <p style={{ fontFamily: F.body, fontSize: 15, color: C.textDim, marginBottom: 32 }}>77 verified providers across Brooklyn. Free to browse.</p>
...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text }}>
      ...
      {page === 'directory' && <DirectoryPage setPage={navigate}/>}
