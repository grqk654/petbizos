      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '52px 24px 0' }}>
        ...
        <div style={{ marginBottom: 52 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
            {tools.map(t => (
              <button key={t.id} onClick={() => setPage('tools/' + t.id)}
...
// ─── TOOLS PAGE ───────────────────────────────────────────────────────────────
const toolSlugMap = {
  'annual':    'Annual Ownership Cost',
  'food':      'Dog Food Calculator',
  'insurance': 'Pet Insurance Value',
  'vet':       'Vet Budget Planner',
  'walking':   'Walker vs Boarding Cost',
  'puppy':     'Puppy First Year',
  'grooming':  'Grooming Cost Estimator',
  'vaccines':  'NYC Vaccine Schedule',
  'sitter':    'Boarding vs Sitter Cost',
  'apartment': 'Apartment Dog Matcher',
}

const ToolsPage = ({ initialTool = 'annual' }) => {
...
export default function App() {
  ...
  const isTools = page === 'tools' || page.startsWith('tools/')
  const toolSlug = page.startsWith('tools/') ? page.replace('tools/', '') : 'annual'
  const validTool = toolSlugMap[toolSlug] ? toolSlug : 'annual'
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text }}>
      ...
      {isTools && <ToolsPage initialTool={validTool}/>}
